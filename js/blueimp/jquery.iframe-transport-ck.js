 ...
 *//*jslint unparam: true, nomen: true *//*global define, window, document */(function(e) {
    ...
    e.ajaxTransport("iframe", function(n) {
        if (n.async) {
            ...
            return {
                send: function(o, u) {
                    r = e('<form style="display:none;"></form>');
                    r.attr("accept-charset", n.formAcceptCharset);
                    s = /\?/.test(n.url) ? "&" : "?";
                    if (n.type === "DELETE") {
                        n.url = n.url + s + "_method=DELETE";
                        n.type = "POST";
                    } else if (n.type === "PUT") {
                        n.url = n.url + s + "_method=PUT";
                        n.type = "POST";
                    } else if (n.type === "PATCH") {
                        n.url = n.url + s + "_method=PATCH";
                        n.type = "POST";
                    }
                    t += 1;
                    i = e('<iframe src="javascript:false;" name="iframe-transport-' + t + '"></iframe>').bind("load", function() {
                        ...
                        i.unbind("load").bind("load", function() {
                            ...
                            e('<iframe src="javascript:false;"></iframe>').appendTo(r);
                            window.setTimeout(function() {
                                r.remove();
                            }, 0);
                        });
                        r.prop("target", i.prop("name")).prop("action", n.url).prop("method", n.type);
                        n.formData && e.each(n.formData, function(t, n) {
                            e('<input type="hidden"/>').prop("name", n.name).val(n.value).appendTo(r);
                        });
                        if (n.fileInput && n.fileInput.length && n.type === "POST") {
                            t = n.fileInput.clone();
                            ...
                            r.append(n.fileInput).prop("enctype", "multipart/form-data").prop("encoding", "multipart/form-data");
                        }
                        r.submit();
                        t && t.length && n.fileInput.each(function(n, r) {
                            var i = e(t[n]);
                            e(r).prop("name", i.prop("name"));
                            i.replaceWith(r);
                        });
                    });
                ...
                abort: function() {
                    i && i.unbind("load").prop("src", "javascript".concat(":false;"));
                    r && r.remove();
                }
            };
        }
